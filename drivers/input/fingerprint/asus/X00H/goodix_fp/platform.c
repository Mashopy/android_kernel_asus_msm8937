...
int gf_parse_dts(struct gf_dev* gf_dev)
{
	...
	gf_dev->reset_gpio = get_reset_gpio_number();
	gf_dev->irq_gpio = get_irq_gpio_number();

	if(!gpio_is_valid(gf_dev->reset_gpio))
	{
		pr_err("get commonfp reset gpio fialed");
		return -1;
	}

	if(!gpio_is_valid(gf_dev->irq_gpio))
	{
		pr_err("get commonfp irq gpio fialed");
		return -1;
	}

	gpio_direction_input(gf_dev->irq_gpio);
	
	pr_warn("gf_spi:gpio use common reset_gpio:%d,irq_gpio:%d\n",gf_dev->reset_gpio,gf_dev->irq_gpio);
